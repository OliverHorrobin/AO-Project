...
function Beam = genGauss(a,b,x,y,xBar,yBar,amplitude,Rotation_angle,noise)

[xx,yy] = meshgrid((linspace(1,x,x)),linspace(1,y,y));

Beam = amplitude * exp(-1.0*(pi * ((xx-xBar).^2.0/a.^2.0 + (yy-yBar).^2.0/b.^2.0)));

% No need to Rotate more than 180 as it just repeats due to the symetrical nature of an oval
%The counterclockwise rotation of the beam in degrees
% Rotation_angle = 15;
...
if noise == 1
   nBeam = zeros(size(Beam));
    ...
    z = nBeam;
    n = rand(1,nnz(c)) * (amplitude/10); %Final number corresponds to the maximum amplitude of the noise(amplitude/#)
    z(c) = n;
    Beam = Beam + z;
elseif noise == 0
    Beam = Beam;
...
figure(1);
imagesc(Beam);% Displays an image of the beam

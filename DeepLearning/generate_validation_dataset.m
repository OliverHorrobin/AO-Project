...
%         line4 = sprintf("Do you wish  to continue ?");
    ...
    Rdir = fullfile('Dataset','Responses','Validation');
    Odir = fullfile('Dataset','Observations','Validation');
    mkdir(Rdir)
    mkdir(Odir)
    ...
    parfor worker = 1:workers-1
        ...
        for i = start_idx:final_idx
            ...
            fdir = fullfile(Rdir,fname);            % Full path to save it to ".\Dataset\Responses\Validation\coeffs_index.csv".
            writematrix(coeffs,fdir);               % Write file
            
            fname = sprintf("farfield_%i.png",i);	% Name of farfield image, "farfield_idex.png"
            fdir = fullfile(Odir,fname);            % Full path to save it to ".\Dataset\Observations\Validation\farfield_idex.png".

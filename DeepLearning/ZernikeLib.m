classdef ZernikeLib
    ...
    properties(Constant, Access = public)
        ...
        % This is ugly, I know. But I want to be able to access the methods
        % and properties without an instance of this class. 
    ...
    methods (Static)
        function [poly,x,y] = polys(ord,res)
            %
            % poly = poly(lib,ord,res)  returns polynomial of order 'ord'
            %   and resolution 'res', where 'res' is the width of x,y vectors
            %
            % [poly,x,y] = poly(lib,ord,res) Also returns the x,y vectors
            %   for plotting mainly
            %
            % ord = integer vector  returns polys of index = ord.
            % e.g.  ord = [0,4,2]   returns polynomials [Z(j=0) Z(j=4) Z(j=2)]
            %       ord = 'all'     returns all polys up to 15th order in
            %                       ascending order
        ...
        function P = circ_aperture(resolution)
            P = zeros(resolution);
